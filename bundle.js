...
var Autoplay = function () {
  function Autoplay(container, initState, callback) {
    var _this = this;
    ...
    this.resetButton = this.container.append('button').attr('class', 'button button-reset').text('↺').on('click', function () {
      _this.state.reload = true;
      _this.stateChange();
    });

    this.playButton = this.container.append('button').attr('class', 'button button-play').on('click', function () {
      _this.state.playing = !_this.state.playing;
      _this.stateChange();
    });

    this.nextButton = this.container.append('button').attr('class', 'button button-next').text('>').on('click', function () {
      _this.state.next = true;
      _this.stateChange();
    });
...
var Plot = function () {
  function Plot(container) {
    ...
    this.svg = d3.select(container).append('svg').attr('width', this.width + this.margin.left + this.margin.right).attr('height', this.height + this.margin.top + this.margin.bottom);
  ...
  _createClass(Plot, [{
    ...
    value: function update(data) {
      ...
      // Data Points
      var points = this.g.selectAll('.point').data(data);

      var pointsEntered = points.enter().append('circle');

      points.merge(pointsEntered).attr('class', 'point').attr('cx', function (d) {
        return _this.xscale(d.x1);
      }).attr('cy', function (d) {
        return _this.yscale(d.x2);
      }).attr('stroke-width', function (d) {
        return _this.highlighted === d ? 3 : 0;
      }).attr('stroke', 'yellow').attr('r', 5).attr('fill', function (d) {
        var color = d.y > 0 ? 'red' : 'blue';
        return color;
      });
      ...
      var lines = void 0;
      if (this.finalLine) {
        lines = this.g.selectAll('.line').data([this.finalLine]);

        var linesEntered = lines.enter().append('line');

        lines.merge(linesEntered).attr('class', 'line').attr('x1', function (d) {
          return _this.xscale(d.a.x);
        }).attr('y1', function (d) {
          return _this.yscale(d.a.y);
        }).attr('x2', function (d) {
          return _this.xscale(d.b.x);
        }).attr('y2', function (d) {
          return _this.yscale(d.b.y);
        }).attr('stroke-width', 1).attr('stroke', 'green');
      } else {
        lines = this.g.selectAll('.line').data(this.lines);

        var _linesEntered = lines.enter().append('line');

        lines.merge(_linesEntered).attr('class', 'line').attr('x1', function (d) {
          return _this.xscale(d.a.x);
        }).attr('y1', function (d) {
          return _this.yscale(d.a.y);
        }).attr('x2', function (d) {
      ...
      this.xAxis.selectAll('.axis--x').call(xAxis);

      this.yAxis.selectAll('.axis--y').call(yAxis);
    }
  }, {
    key: 'initAxis',
    value: function initAxis() {
      this.xAxis = this.g.append('g').attr('class', 'axis axis--x').attr('transform', 'translate(0, ' + this.height + ')').call(d3.axisBottom(this.xscale));

      this.yAxis = this.g.append('g').attr('class', 'axis axis--y').attr('transform', 'translate(0, 0)').call(d3.axisLeft(this.yscale));
    }
  }, {
    key: 'addLine',
    value: function addLine(weights) {
      var final = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;

      var k = -(weights[1] / weights[2]);
      var d = -(weights[0] / weights[2]);
      var a = { x: -1, y: d - k };
      var b = { x: 1, y: d + k };
      if (final) this.finalLine = { a: a, b: b };else this.lines.push({ a: a, b: b });
    }
  }, {
    key: 'highlightPoint',
...
var Slider = function Slider(container, opts, onchange) {
  ...
  this.slider = d3.select(container).append('svg').attr('width', this.width + this.margin.left + this.margin.right).attr('height', this.height).append('g').attr('transform', 'translate(' + this.margin.left + ', ' + this.height / 2 + ')');

  this.scale = d3.scaleLinear().domain([min, max]).range([0, this.width]).clamp(true);

  // Slider Track + Drag Event
  this.slider.append('line').attr('class', 'track').attr('x1', this.scale.range()[0]).attr('x2', this.scale.range()[1]).select(function () {
    return this.parentNode.appendChild(this.cloneNode(true));
  }).attr('class', 'track-overlay').call(d3.drag().on('start drag', function () {
    _this.handle.attr('cx', _this.scale(_this.scale.invert(d3.event.x)));
    onchange(_this.scale.invert(d3.event.x));
  }));

  // Ticks
  this.slider.insert('g', '.track-overlay').attr('transform', 'translate(0, 20)').selectAll('text').data(this.scale.ticks(10)).enter().append('text').attr('class', 'tick').attr('x', this.scale).attr('text-anchor', 'middle').text(function (d) {
    return '' + d + _this.unit;
  });

  // Handle
  this.handle = this.slider.insert('circle', '.track-overlay').attr('class', 'handle').attr('cx', this.scale(init)).attr('r', 9);
};
...
exports.resetRandomValue = resetRandomValue;
...
function targetFunction(point) {
  var seed = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : randomValue;
...
function generateRandomPoint() {
  ...
  var x1 = Math.random() * Math.abs(min - max) + min;
  var x2 = Math.random() * Math.abs(min - max) + min;
  return {
    x1: x1,
    x2: x2,
    y: targetFunction({ x1: x1, x2: x2 })
  };
}
...
var N = 100;
var data = (0, _data.generateDataset)(N);
...
function adjustWeights(weights, mismatch) {
  return [weights[0] + mismatch.y, weights[1] + mismatch.x1 * mismatch.y, weights[2] + mismatch.x2 * mismatch.y];
}
...
var plot = new _Plot2.default('#plot', {
  width: 800,
  height: 480,
  margin: {
    top: 40,
...
function iterate() {
  ...
  if (mismatches.length !== 0) {
    var randomMismatch = mismatches[Math.floor(Math.random() * mismatches.length)];
...
function reload() {
  (0, _data.resetRandomValue)();
  data = (0, _data.generateDataset)(N);
  plot.reset();
  plot.update(data);
  iterate();
}
...
var sliderDelay = new _Slider2.default('#slider-delay', {
  min: 0,
  max: 1000,
  init: delay,
  width: 600,
...
var mismatch = void 0;
var autoplay = new _Autoplay2.default('#autoplay', { playing: playing }, function (state) {
